...
export const CustomCheckbox = ({ size = 24, label }: Props) => {
  ...
  return (
    <div className={s.container}>
      <label htmlFor={checkBoxId} onClick={changeStatus} className={s.label}>
        {label}
      </label>
      <div id={checkBoxId} className={s.checkbox} onClick={changeStatus} style={{ width: size, height: size }}>
        <div className={`${s.innerSquare} ${qrExcavate ? s.checked : ''}`} />
      </div>
    </div>
  )
}
